...
if(old_rank==0){
    ...
    typedef int slice_original[n_y][n_z];
    typedef int slice_partial[m_y][m_z];
    ...
    if(my_rank==0){
    original_matrix_1 = (slice_original *)malloc((huge_size)*sizeof(int));
    original_matrix_2 = (slice_original *)malloc((huge_size)*sizeof(int));
    }
    size_t partial_huge_size= m_x*m_y*m_z;
    partial_matrix_1 = (slice_partial *)malloc((partial_huge_size)*sizeof(int));
    partial_matrix_2 = (slice_partial *)malloc((partial_huge_size)*sizeof(int));

//initialization through multiple cores, count down till find the nearest multiple
...
// initialization strips with random numbers
     size_t init_size= (huge_size)/n_initialization_cores;
     typedef int init_slice[1][1];
     init_slice *partial_init_1;
     init_slice *partial_init_2;
     partial_init_1 = (init_slice*)malloc((init_size)*sizeof(int));
     partial_init_2 = (init_slice*)malloc((init_size)*sizeof(int));


if(my_rank<n_initialization_cores){
     for (size_t i=0;i<init_size;++i){
     	partial_init_1[0][0][i]= rand() % 10;
     	partial_init_2[0][0][i]= rand() % 10;
     }
}
...
if(my_rank<n_initialization_cores){
MPI_Gather(&(partial_init_1[0][0][0]), init_size, MPI_INT, &(original_matrix_1[0][0][0]), init_size, MPI_INT, 0, init_communicator);
MPI_Gather(&(partial_init_2[0][0][0]), init_size, MPI_INT, &(original_matrix_2[0][0][0]), init_size, MPI_INT, 0, init_communicator);
}
...
//	for(int j=0;j<n_y;++j){
//		for(int k=0;k<n_z;++k){
//	     	//	original_matrix_1[i][j][k]=0;
//		//	original_matrix_2[i][j][k]=0; 
//		}
//	}
//}
//}
...
int starts[3]={0,0,0};
MPI_Type_create_subarray(3, aumented_dim, local_dim, starts, MPI_ORDER_C, MPI_INT, &block_partial);
MPI_Type_create_resized(block_partial, 0, m_z*sizeof(int), &resized_block);
...
if(my_rank==0){
original_matrix_1=realloc(original_matrix_1,(aum_huge_size)*sizeof(int));
original_matrix_2=realloc(original_matrix_2,(aum_huge_size)*sizeof(int));
}

//Sending partial blocks to every available core
MPI_Scatterv(&(original_matrix_1[0][0][0]), counts, displaces, resized_block, &(partial_matrix_1[0][0][0]), partial_huge_size, MPI_INT, 0, grid_communicator);
MPI_Scatterv(&(original_matrix_2[0][0][0]), counts, displaces, resized_block, &(partial_matrix_2[0][0][0]), partial_huge_size, MPI_INT, 0, grid_communicator);
...
// NOW GATHER!
MPI_Gatherv(&(partial_matrix_1[0][0][0]), partial_huge_size, MPI_INT, &(original_matrix_1[0][0][0]), counts, displaces, resized_block, 0, grid_communicator);

//free datatype, wont be used anymore
MPI_Type_free(&resized_block);
if(my_rank==0){
//back to original size
original_matrix_1=realloc(original_matrix_1, (huge_size)*sizeof(int));

//printf("\nhere is your matrix:\n");
//for(int i=0;i<n_x;++i){
//	for(int j=0;j<n_y;++j){
//		for(int k=0;k<n_z;++k){
//			printf("%d ",original_matrix_1[i][j][k]);}}}
